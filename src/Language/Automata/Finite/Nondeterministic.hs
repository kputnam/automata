...
step m s = fold (union . free m) empty . read m s . Just

-- Run the simulation, producing True if the machine accepted the input
-- or False otherwise.
--
eval :: (Ord a, Ord t) => NFA a t -> [t] -> Bool
eval m = accept . foldl' step' (ap free start m)
  where
    ...
    loop = loop' ([], [])
      where
        ...
        loop' (ts, as) done (s@(State a x _):ss)
          | a `member` done = loop' (ts, as) done ss
